...
我是从\ Python_\ 迁移过来的，在我看来——\ ES5_\ 版本的语法过于复杂，在语法上\ ES6_\ 版本同\ Python_\ 、\ Ruby_\ 有了更多的共同特性。虽然在\ ES6_\ 版本中箭头函数、类与继承的实现只是一种语法糖，但是使用起来更为自然。块级作用域、模板字符串、解构赋值、\ ``Map``\ 与\ ``Set``\ 、迭代器与生成器、\ ``Promise``\ 、代理、模块化，这些新特性使代码的书写变得更为优雅。
